...
### Image Splitter v2.6
**发布日期**: 2026-07-19
- 新增：自定义页面导出范围、"操作全部"批量修改开关、快捷翻页(◀/▶)按钮。
- 优化：引入多线程异步保存避免保存大文件时界面假死、状态栏提示常驻与防抖处理。
- 优化：鼠标滚轮缩放体验更加流畅，解决了因重绘事件堆积导致的 UI 界面假死和严重卡顿问题。
- 修复：切图算法在后台线程强制读取 UI 控件引发的程序崩溃闪退隐患。
...
**新增功能**:
- 图像自动适应窗口
- 打开大尺寸图片或切换页面时，默认缩放至“适应窗口”显示，避免图像溢出画布，大幅提升用户体验。
- 新增“适应窗口”按钮
- 在缩放控制区新增“适应”按钮，可随时一键恢复图像到适应窗口状态。
- 精确中心缩放
- 使用鼠标滚轮或缩放按钮时，以鼠标指针位置为中心进行缩放（类似 Photoshop），操作更直观。
- 实时缩放比例显示
- 在缩放按钮旁显示当前缩放百分比（如 “85%”），方便用户掌握当前视图比例。
  
**修复问题**:
- 修复首次加载大图时图像超出窗口边界的问题（致命体验问题）。
- 修复切换页面后未重置缩放与偏移量导致显示错位的问题。
- 修复在极小窗口下图像尺寸计算异常导致崩溃的问题。
- 修复拖拽文件失败时无提示的问题（降级模式下仍保持稳定运行）。
